...
def create_image(options: Options) -> torch.Tensor:
    ...
    """
    r = [[0.5021, 0.2843, 0.1935], [0.8017, 0.5914, 0.7038]]
    g = [[0.1138, 0.0684, 0.5483], [0.8733, 0.6004, 0.5983]]
    b = [[0.9047, 0.6829, 0.3117], [0.6258, 0.2893, 0.9914]]
    rgb = [r, g, b]
...
def lin_layer_forward(weights: torch.Tensor, random_image: torch.Tensor) -> torch.Tensor:
    ...
    inj = torch.dot(weights, random_image)

    """inj = weights * random_image    
    deze methode werkte nie helemaal want ge moet nogeens alle waardes 
    in de output tensor met elkaar optellen om 1 getal te bekomen"""
