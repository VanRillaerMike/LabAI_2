...
def train_lin_model(model: LinearRegression, optimizer: torch.optim.Optimizer,
    ...
    for epoch in range(options.num_epochs):
        for data in train_data:
            ...
            predicted_price = model.forward(size)                   # eigen functie, sizes door ons model sturen en met de outut iets doen
            # calculate the loss, use your self created mse loss
            loss = mse(predicted_price, price)                      # berekenen hoe verkeerd we zitten van de gewenste waarden (in prijs)
            # As mentioned before, the grads always needs to be zeroed before backprop (use your optimizer to do this)
            optimizer.zero_grad()                                   # zie documentatie optimizers
...
def train_classification_model(model: Classifier, optimizer: torch.optim.Optimizer,
    ...
    for epoch in range(options.num_epochs):
        ...
        for x, y in dataset.train_loader:
            ...
            prediction = model.forward(x_reshaped)                   # eigen functie, datasetwaarden door ons model sturen en met de output iets doen
